...
export default function PropertyDetail({ params, searchParams }: { params: { id: string }; searchParams: { lang?: string } }) {
  ...
  const t = copy[locale];
  const propertyName = text(property.title, locale);
  const detailItems = [
        ...
        <div className="min-w-0">
          <div className="mb-8 flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
            <div>
              ...
              <h1 className="mt-3 max-w-4xl text-4xl font-semibold leading-tight tracking-tight md:text-6xl">{propertyName}</h1>
              <p className="mt-4 text-lg text-night/55">{property.priceNote}</p>
            </div>
            <div className="w-full shrink-0 md:w-40">
              <PropertyInquiryButton propertyId={property.id} propertyName={propertyName} locale={locale} />
            </div>
          </div>
