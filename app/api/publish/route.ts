...
const inquiryApiUrl = process.env.NEXT_PUBLIC_CMS_API_URL || "https://tokyostay-cms-gshaiql6.edgeone.cool";
...
const labels = {
  en: {
    ...
    inquiryLead: "Send your request without exposing public contact details.",
    email: "Email",
    messenger: "Messenger ID or contact note",
    message: "Message",
    submit: "Send inquiry",
  ...
  zh: {
    ...
    inquiryTitle: "私密咨询",
    inquiryLead: "客户可通过表单咨询，前台不公开微信、LINE 或电话。",
    email: "邮箱",
    messenger: "微信 / LINE / WhatsApp 或备注",
    message: "咨询内容",
    submit: "提交咨询",
  ...
  ja: {
    ...
    video: "ルームツアー",
    map: "地図",
    private: "公開ページに連絡先は表示しません。",
    inquiryTitle: "お問い合わせ",
    inquiryLead: "連絡先を公開せず、フォームからお問い合わせできます。",
    email: "メール",
    messenger: "連絡先IDまたは備考",
    message: "お問い合わせ内容",
    submit: "送信",
...
function inquiryScript(propertyId: string) {
  return `<script>document.querySelectorAll('[data-inquiry-form]').forEach(form=>{form.addEventListener('submit',async event=>{event.preventDefault();const lang=localStorage.getItem('tokyostayLang')||'en';const data=Object.fromEntries(new FormData(form).entries());data.propertyId='${esc(propertyId)}';const result=form.querySelector('.result');result.textContent='Sending...';try{const res=await fetch('${inquiryApiUrl.replace(/\/$/, "")}/api/inquiry',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(data)});const json=await res.json();result.textContent=json.thankYou?.[lang]||json.thankYou?.en||'Sent';form.reset()}catch(error){result.textContent='Submit failed. Please try again later.'}})});</script>`;
}
